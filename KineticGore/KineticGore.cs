...
namespace KineticGore
{
		...
		public override bool PreKill(NPC npc)
		{
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				if (IsHumanoid(npc) && Main.rand.Next(100) < config.DismembermentChance)
				{
					...
					Projectile.NewProjectile(npc.GetSource_Death(), npc.Center, headVelocity, projType, 0, 0, Main.myPlayer, (float)npc.type, headAI, packedColor);
				...
				Projectile.NewProjectile(npc.GetSource_Death(), npc.Center, launchVelocity, projType, 0, 0, Main.myPlayer, (float)npc.type, packedAI, packedColor);
			}
			return true;
		}
	}
	...
	public class PhysicsCorpse : ModProjectile
	{
		...
		private void OnHitByPlayer(int hitDir)
		{
			...
			if (IsHumanoid() && dismemberType == 0 && Main.netMode != NetmodeID.MultiplayerClient && Main.rand.Next(100) < config.DismembermentChance)
			{
				if (Main.rand.NextBool())
				{
					...
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), spawnPos, headLaunch, projType, 0, 0, Main.myPlayer, Projectile.ai[0], headAI, Projectile.ai[2]);
				...
				else
				{
					...
					// Spawn Legs
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, legsLaunch, projType, 0, 0, Main.myPlayer, Projectile.ai[0], legsAI, Projectile.ai[2]);
		...
		private void Splatter()
		{
			...
			try
			{
				int type = (int)Projectile.ai[0]; // Use explicit local var to avoid scope issues
				NPC dummy = new NPC();
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			if (IsSlime())
			{
				...
				Main.EntitySpriteDraw(texture, drawPos, fullSrc, drawColor, Projectile.rotation, origin, new Vector2(scaleX, scaleY), SpriteEffects.None, 0);
				return false;
			}
